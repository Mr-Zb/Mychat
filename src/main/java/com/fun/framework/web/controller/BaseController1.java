...
public abstract class BaseController1 {
    ...
    public Integer getAuthentication(HttpServletRequest request) throws TokenException {
        ...
        Integer userId = (Integer) redisTemplate.opsForHash().get(Constants.ADMINTOKEN,token);
        if (userId==null){
    ...
    public ModelAndView runtimeExceptionHandler(Throwable throwable) {
        ...
        if ((throwable instanceof BusinessException)) {
            ...
            if (throwable.getMessage() != null){
                //jsonView.addStaticAttribute("info", throwable.getMessage().replace("msg:",""));
                jsonView.addStaticAttribute("info", "亲，服务器开小差了，请重试！");
            }
        }
        if (this.logger.isErrorEnabled()) {
            this.logger.error("服务器异常", throwable);
        }
